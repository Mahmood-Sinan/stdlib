    ...
    subroutine test_kabsch_umeyama_real(error)
        ...
        block
            ...
            call check(error, all_close(P_recovered, P_original, abs_tol = sptol), .true., &
                        "Kabsch_Umeyama real(sp) failed: Random transformation, P_recovered not equal to P_original")
            if(allocated(error)) return
            call check(error, is_close(rmsd, zero_sp, abs_tol = sptol), .true., &
                        "Kabsch_Umeyama real(sp) failed: Random transformation, RMSD not equal to zero")
            if(allocated(error)) return
            ...
            call check(error, all_close(R_recovered, Identity, abs_tol=sptol), .true., &
                        "Kabsch_Umeyama real(sp) failed: Identity transformation, R_recovered not equal to Identity")
            if(allocated(error)) return
            call check(error, all_close(t_recovered, zero_t, abs_tol = sptol), .true., &
                        "Kabsch_Umeyama real(sp) failed: Identity transformation, t_recovered not equal to zero")
            if(allocated(error)) return
            call check(error, is_close(rmsd, zero_sp, abs_tol = sptol), .true., &
                        "Kabsch_Umeyama real(sp) failed: Identity transformation, RMSD not equal to zero")
            if(allocated(error)) return
            ...
            call check(error, all_close(P_recovered, P_original, abs_tol = sptol), .true., &
                        "Kabsch_Umeyama real(sp) failed: Random transformation (scale disabled), P_recovered not equal to&
                            & P_original")
            if(allocated(error)) return
            call check(error, is_close(c_recovered, one_sp, abs_tol = sptol), .true., &
                        "Kabsch_Umeyama real(sp) failed: Random transformation (scale disabled), c_recovered not equal to one")
            if(allocated(error)) return
            call check(error, is_close(rmsd, zero_sp, abs_tol = sptol), .true., &
                        "Kabsch_Umeyama real(sp) failed: Random transformation (scale disabled), RMSD not equal to zero")
            if(allocated(error)) return
            ...
            call check(error, is_close(det(R_recovered), one_sp, abs_tol=sptol), .true., &
                        "Kabsch_Umeyama real(sp) failed: Random transformation (weighted), det(R_recovered) not equal to one")
            if(allocated(error)) return
            call check(error, all_close(P_recovered, P_original, abs_tol = sptol), .true., &
                        "Kabsch_Umeyama real(sp) failed: Random transformation (weighted), P_recovered not equal to P_original")
            if(allocated(error)) return
            call check(error, is_close(rmsd, zero_sp, abs_tol = sptol), .true., &
                        "Kabsch_Umeyama real(sp) failed: Random transformation (weighted), RMSD not equal to zero")
            if(allocated(error)) return
        end block
        block
            integer, parameter :: d = 3, N = 8
            ...
            call check(error, all_close(P_recovered, P_original, abs_tol = dptol), .true., &
                        "Kabsch_Umeyama real(dp) failed: Random transformation, P_recovered not equal to P_original")
            if(allocated(error)) return
            call check(error, is_close(rmsd, zero_dp, abs_tol = dptol), .true., &
                        "Kabsch_Umeyama real(dp) failed: Random transformation, RMSD not equal to zero")
            if(allocated(error)) return
            ...
            call check(error, all_close(R_recovered, Identity, abs_tol=dptol), .true., &
                        "Kabsch_Umeyama real(dp) failed: Identity transformation, R_recovered not equal to Identity")
            if(allocated(error)) return
            call check(error, all_close(t_recovered, zero_t, abs_tol = dptol), .true., &
                        "Kabsch_Umeyama real(dp) failed: Identity transformation, t_recovered not equal to zero")
            if(allocated(error)) return
            call check(error, is_close(rmsd, zero_dp, abs_tol = dptol), .true., &
                        "Kabsch_Umeyama real(dp) failed: Identity transformation, RMSD not equal to zero")
            if(allocated(error)) return
            ...
            call check(error, all_close(P_recovered, P_original, abs_tol = dptol), .true., &
                        "Kabsch_Umeyama real(dp) failed: Random transformation (scale disabled), P_recovered not equal to&
                            & P_original")
            if(allocated(error)) return
            call check(error, is_close(c_recovered, one_dp, abs_tol = dptol), .true., &
                        "Kabsch_Umeyama real(dp) failed: Random transformation (scale disabled), c_recovered not equal to one")
            if(allocated(error)) return
            call check(error, is_close(rmsd, zero_dp, abs_tol = dptol), .true., &
                        "Kabsch_Umeyama real(dp) failed: Random transformation (scale disabled), RMSD not equal to zero")
            if(allocated(error)) return
            ...
            call check(error, is_close(det(R_recovered), one_dp, abs_tol=dptol), .true., &
                        "Kabsch_Umeyama real(dp) failed: Random transformation (weighted), det(R_recovered) not equal to one")
            if(allocated(error)) return
            call check(error, all_close(P_recovered, P_original, abs_tol = dptol), .true., &
                        "Kabsch_Umeyama real(dp) failed: Random transformation (weighted), P_recovered not equal to P_original")
            if(allocated(error)) return
            call check(error, is_close(rmsd, zero_dp, abs_tol = dptol), .true., &
                        "Kabsch_Umeyama real(dp) failed: Random transformation (weighted), RMSD not equal to zero")
            if(allocated(error)) return
        end block
    end subroutine
    ...
    subroutine test_kabsch_umeyama_complex(error)
        ...
        block
            ...
            call check(error, is_close(abs(det(R_recovered)), one_sp, abs_tol=sptol), .true., &
                        "Kabsch_Umeyama complex(sp) failed: Random transformation, abs(det(R_recovered)) not equal to one")
            if(allocated(error)) return
            call check(error, all_close(P_recovered, P_original, abs_tol = sptol), .true., &
                        "Kabsch_Umeyama complex(sp) failed: Random transformation, P_recovered not equal to P_original")
            if(allocated(error)) return
            call check(error, is_close(rmsd, zero_sp, abs_tol = sptol), .true., &
                        "Kabsch_Umeyama complex(sp) failed: Random transformation, RMSD not equal to zero")
            if(allocated(error)) return
            ...
            call kabsch_umeyama(Q_original, Q_original, R_recovered, t_recovered, c_recovered, rmsd)
            call check(error, all_close(R_recovered, Identity, abs_tol=sptol), .true., &
                        "Kabsch_Umeyama complex(sp) failed: Identity transformation, R_recovered not equal to Identity")
            if(allocated(error)) return
            call check(error, all_close(t_recovered, zero_t, abs_tol = sptol), .true., &
                        "Kabsch_Umeyama complex(sp) failed: Identity transformation, t_recovered not equal to zero")
            if(allocated(error)) return
            call check(error, is_close(rmsd, zero_sp, abs_tol = sptol), .true., &
                        "Kabsch_Umeyama complex(sp) failed: Identity transformation, RMSD not equal to zero")
            if(allocated(error)) return
            ...
            call check(error, is_close(abs(det(R_recovered)), one_sp, abs_tol=sptol), .true., &
                        "Kabsch_Umeyama complex(sp) failed: Random transformation (scale disabled), abs(det(R_recovered)) not&
                            & equal to one")
            if(allocated(error)) return
            call check(error, all_close(P_recovered, P_original, abs_tol = sptol), .true., &
                        "Kabsch_Umeyama complex(sp) failed: Random transformation (scale disabled), P_recovered not equal to&
                            & P_original")
            if(allocated(error)) return
            call check(error, is_close(c_recovered, one_csp, abs_tol = sptol), .true., &
                        "Kabsch_Umeyama complex(sp) failed: Random transformation (scale disabled), c_recovered not equal to one")
            if(allocated(error)) return
            call check(error, is_close(rmsd, zero_sp, abs_tol = sptol), .true., &
                        "Kabsch_Umeyama complex(sp) failed: Random transformation (scale disabled), RMSD not equal to zero")
            if(allocated(error)) return
            ...
            call check(error, is_close(abs(det(R_recovered)), one_sp, abs_tol=sptol), .true., &
                        "Kabsch_Umeyama complex(sp) failed: Random transformation (weighted), abs(det(R_recovered)) not equal to&
                            & one")
            if(allocated(error)) return
            call check(error, all_close(P_recovered, P_original, abs_tol = sptol), .true., &
                        "Kabsch_Umeyama complex(sp) failed: Random transformation (weighted), P_recovered not equal to P_original")
            if(allocated(error)) return
            call check(error, is_close(rmsd, zero_sp, abs_tol = sptol), .true., &
                        "Kabsch_Umeyama complex(sp) failed: Random transformation (weighted), RMSD not equal to zero")
            if(allocated(error)) return
        end block
        block
            integer, parameter :: d = 3, N = 8
            ...
            call check(error, is_close(abs(det(R_recovered)), one_dp, abs_tol=dptol), .true., &
                        "Kabsch_Umeyama complex(dp) failed: Random transformation, abs(det(R_recovered)) not equal to one")
            if(allocated(error)) return
            call check(error, all_close(P_recovered, P_original, abs_tol = dptol), .true., &
                        "Kabsch_Umeyama complex(dp) failed: Random transformation, P_recovered not equal to P_original")
            if(allocated(error)) return
            call check(error, is_close(rmsd, zero_dp, abs_tol = dptol), .true., &
                        "Kabsch_Umeyama complex(dp) failed: Random transformation, RMSD not equal to zero")
            if(allocated(error)) return
            ...
            call kabsch_umeyama(Q_original, Q_original, R_recovered, t_recovered, c_recovered, rmsd)
            call check(error, all_close(R_recovered, Identity, abs_tol=dptol), .true., &
                        "Kabsch_Umeyama complex(dp) failed: Identity transformation, R_recovered not equal to Identity")
            if(allocated(error)) return
            call check(error, all_close(t_recovered, zero_t, abs_tol = dptol), .true., &
                        "Kabsch_Umeyama complex(dp) failed: Identity transformation, t_recovered not equal to zero")
            if(allocated(error)) return
            call check(error, is_close(rmsd, zero_dp, abs_tol = dptol), .true., &
                        "Kabsch_Umeyama complex(dp) failed: Identity transformation, RMSD not equal to zero")
            if(allocated(error)) return
            ...
            call check(error, is_close(abs(det(R_recovered)), one_dp, abs_tol=dptol), .true., &
                        "Kabsch_Umeyama complex(dp) failed: Random transformation (scale disabled), abs(det(R_recovered)) not&
                            & equal to one")
            if(allocated(error)) return
            call check(error, all_close(P_recovered, P_original, abs_tol = dptol), .true., &
                        "Kabsch_Umeyama complex(dp) failed: Random transformation (scale disabled), P_recovered not equal to&
                            & P_original")
            if(allocated(error)) return
            call check(error, is_close(c_recovered, one_cdp, abs_tol = dptol), .true., &
                        "Kabsch_Umeyama complex(dp) failed: Random transformation (scale disabled), c_recovered not equal to one")
            if(allocated(error)) return
            call check(error, is_close(rmsd, zero_dp, abs_tol = dptol), .true., &
                        "Kabsch_Umeyama complex(dp) failed: Random transformation (scale disabled), RMSD not equal to zero")
            if(allocated(error)) return
            ...
            call check(error, is_close(abs(det(R_recovered)), one_dp, abs_tol=dptol), .true., &
                        "Kabsch_Umeyama complex(dp) failed: Random transformation (weighted), abs(det(R_recovered)) not equal to&
                            & one")
            if(allocated(error)) return
            call check(error, all_close(P_recovered, P_original, abs_tol = dptol), .true., &
                        "Kabsch_Umeyama complex(dp) failed: Random transformation (weighted), P_recovered not equal to P_original")
            if(allocated(error)) return
            call check(error, is_close(rmsd, zero_dp, abs_tol = dptol), .true., &
                        "Kabsch_Umeyama complex(dp) failed: Random transformation (weighted), RMSD not equal to zero")
            if(allocated(error)) return
        end block
    end subroutine
